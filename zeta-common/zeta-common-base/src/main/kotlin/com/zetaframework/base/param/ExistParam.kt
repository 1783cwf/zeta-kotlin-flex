...
class ExistParam<Entity, Id> private constructor() {
     ...
     * ```
     *
     * @param service IService<BaseEntity>   service
    ...
    fun isExist(
        service: IService<Entity>,
        idField: String? = "id",
        isToUnderlineCase: Boolean = true,
    ): Boolean {
        ...
        // 构造查询条件
        val queryWrapper = QueryWrapper.create().eq(fieldName, value)
        ...
        // 验证字段是否存在
        return service.exists(queryWrapper)
    }
}
